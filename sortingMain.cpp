...
#include "sortingStudent.cpp"
#include "sortingTeacher.cpp"
...
int main()
{
  ...
  cout<<"How would you like to sort the columns?"<<endl;
  cout<<"enter 'question' or 'students': "<<endl;
  cin>>column;
  cout<<"Would you like to sort in ascending or decending order?"<<endl;
  cin>>direction;
  //students or question
  t.sort(inFile, column, direction);
  inFile.close();
  return 0;
}
...
string randomTopic()
{
  string topic[9] = {"arrays", "linkedlists", "stacks", "queues", "binarytrees", "btrees", "multimaps", "heaps", "priorityqueues"};
  int i = rand() % 9;
  return topic[i];
}
